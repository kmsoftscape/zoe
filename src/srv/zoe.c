...
void fzoeShut(void)
{
	...
	for(i=0;i<count;i++)
	{
		...
		result=sigqueue(atoi(serviceline),SIGRTMIN+0,(union sigval)0);
		if(result==-1)
		{
...
void fzoeList(void)
{
	...
	for(i=0;i<count;i++)
	{
		...
		printf("\033[0;31m管理线程[%s]\033[0;37m\n",serviceline);

		result=sigqueue(atoi(serviceline),SIGRTMIN+1,(union sigval)0);
		if(result==-1)
		{
			mlogError("sigqueue",errno,strerror(errno),"[%d]",atoi(serviceline));
			continue;
		...
		while(1)
		{
			...
			if(status==0X00&&nature==0X7F)
				printf("\033[0;32m工作线程[%05d]-类型[普通]-状态[空闲]\033[0;37m\n",linuxid);
			else
			if(status==0X01&&nature==0X7F)
				printf("\033[0;31m工作线程[%05d]-类型[普通]-状态[忙碌]\033[0;37m\n",linuxid);
			else
			if(status==0X00&&nature!=0X7F)
				printf("\033[0;32m工作线程[%05d]-类型[批量]-状态[空闲]\033[0;37m\n",linuxid);
			else
			if(status==0X01&&nature!=0X7F)
				printf("\033[0;31m工作线程[%05d]-类型[批量]-状态[忙碌]\033[0;37m\n",linuxid);
		}
		close(fifoid);
	}
...
void *fzoeEmployBoot(void *argument)
{
	...
	if(result!=0)
	{
		mlogError("pthread_setspecific",result,strerror(result),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
		return NULL;
	}
	...
	{
		mlogError("malloc",0,"0","[%d][%X][%d]",vzoePkgSize,vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
		return NULL;
	}
	...
	if(result==-1)
	{
		mlogError("timer_create",errno,strerror(errno),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
		return NULL;
	}
	...
	{
		if(id==SIGALRM)
			flogAnyhow("++++++[%d]",siginfo->si_value.sival_int);
		else
			flogAnyhow("======[%d]",id);
		int index;
	...
	if(result==-1)
	{
		mlogError("sigfillset",errno,strerror(errno),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
		return NULL;
	}
	action.sa_flags=0;
	action.sa_flags|=SA_RESTART;
	...
	if(result==-1)
	{
		mlogError("sigaction",errno,strerror(errno),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
		return NULL;
	}
	...
	if(vzoeThrList[index].nature==0X7F)
	{
		while(1)
		{
			...
			if(result!=0)
			{
				mlogError("pthread_mutex_lock",result,strerror(result),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
				return NULL;
			}
			while(vzoeTskHead==vzoeTskTail&&vzoeThrList[index].button!=0X01)
			{
				...
				if(result!=0)
				{
					mlogError("pthread_cond_wait",result,strerror(result),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
					pthread_mutex_unlock(&vzoeMutex);
					return NULL;
				}
			}
			if(vzoeThrList[index].button==0X01)
			{
				...
				if(result!=0)
				{
					mlogError("pthread_mutex_unlock",result,strerror(result),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
					return NULL;
				}
			...
			if(result!=0)
			{
				mlogError("pthread_cond_signal",result,strerror(result),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
				pthread_mutex_unlock(&vzoeMutex);
				return NULL;
			}
			result=pthread_mutex_unlock(&vzoeMutex);
			...
			if(error!=0)
			{
				...
				mlogEvent("调用错误处理函数[%d]结束",error);
			}
			mlogEvent("交易流程结束",flogTime(1),tv.tv_usec);
	...
	else
	{
		while(1)
		{
			...
			if(dlhand==NULL&&strstr(dlerror(),"undefined symbol")==NULL)
			{
				mlogError("dlsym",0,dlerror(),"[%X][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid);
				goto tag25;
			}
			if(dlhand!=NULL)
			{
			...
			if(trn==NULL)
			{
				mlogError("bsearch",0,"0","[%X][%d][%s][%d]",vzoeThrList[index].posixid,vzoeThrList[index].linuxid,mark,vzoeTrnCount);
				goto tag25;
			}
